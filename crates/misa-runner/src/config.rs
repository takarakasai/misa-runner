...
pub struct WbcConfig {
    ...
    /// そのまま渡る。**`[wbc]` に置いてあるが WBC 専用ではない** —
    /// `torque_ceiling` と同じく `wbc.enabled` に関係なく効くので、開ループ
    /// の重力補償（`control.gravity_feedforward`）にも掛かる。トルクの上限を
    /// 1 か所にまとめるためにここにある。
    ///
    /// # なぜ要るのか
    ///
    /// 絶対値の上限だけでは「1 周期で −40 → +40」が素通りする。階層 QP は
    /// **接地集合が変わる周期に 1 周期だけ矛盾した解**を返すことがあり
    /// （接地力の合計 0.1 N に対して胴体の角加速度 57 rad/s²）、keel では
    /// 踏み替えのたび（trot の半周期 0.2 s ごと）に τ が 145 N·m/周期
    /// = 29,000 N·m/s 跳んでいた。
    ///
    /// 解そのものを弾くのは [`Self::solution_check`] の仕事で、**こちらは
    /// それを抜けたものを受ける最後の砦**。両方入れること。
    ///
    /// # 決め方
    ///
    /// **「1 周期でその機体の立ち姿勢ぶんのトルクが変わってよいか」**で
    /// 考える。keel は膝に約 19.6 N·m 要るので、制御周期 5 ms なら
    /// `19.6 / 0.005 ≈ 4000`。機体の大きさで変わるので既定は無効。
    pub max_torque_rate_nm_s: f64,
    /// **解を採用する前に物理で検算するか。** 既定は有効。
    ///
    /// 階層 QP（HoQP）は優先度 0 の不等式を最小二乗で解くので、**拘束集合の
    /// ランクが変わる周期に「何も満たさない解」を返すことがある**。keel で
    /// 実測した壊れ方は、接地力の合計が 0.1 N（体重 523 N）なのに胴体の
    /// 角加速度が 57 rad/s²、というもの。運動方程式と矛盾している。
    ///
    /// 数値が有限かどうかだけでは捕まらない（NaN ではなく、**もっともらしい
    /// 桁の嘘**が出る）ので、接地力の合計を体重と突き合わせる。外れた周期は
    /// 解を捨てて**重力補償トルクだけ**に落とす（脱力よりは崩れない）。
    ///
    /// 検算が落ちるのは 1 周期だけのはずで、**続くなら設定かモデルが疑わしい**
    /// （接地フラグ・摩擦・トルク上限・質量）。
    pub solution_check: bool,
    /// 検算の下限。接地力の合計がこの倍率 × 体重を下回ったら解を捨てる。
    ///
    /// **立脚が 1 本でもあるときだけ見る。** 4 脚とも浮いていれば接地力 0 が
    /// 正しい。既定 0.2 は「体重の 2 割も支えていないのに立脚がある」を
    /// 異常と見る値で、keel の正常時は 0.91〜1.15 だった。
    pub check_grf_min_frac: f64,
    /// 検算の上限。既定 3.0（着地の衝撃で体重の 2 倍前後までは出る）。
    pub check_grf_max_frac: f64,
    /// **優先度 0 の等式が満たされたかのしきい値**（相対残差）。`0` で無効。
    ///
    /// `‖A·x − b‖ / ‖b‖` を解の後に測る。階層 LSQ は**不等式をスラックで
    /// 逃がす**ので、満たせなかったことは解の値を見ても分からない。等式
    /// （浮遊ベースの運動方程式と「立脚足は動かない」）の残差が、解が破綻
    /// したことの**唯一の直接証拠**になる。
    ///
    /// 接地力を体重と突き合わせる [`Self::check_grf_min_frac`] と違って、
    /// **機体にも質量にも重みにも依らない。** keel の実測では中央 4e-9、
    /// 99.9 % が 1.2e-6、壊れた周期だけ 1.9e-5 と 16 倍離れており、
    /// 1e-6 で接地力の検査とまったく同じ 2 周期を弾いた。
    ///
    /// **既定は無効。** 弾いた周期は重力補償へ落ちるので軌道が変わり、
    /// それが次の周期の残差を変える（測っただけの分布より弾く回数が増える）。
    /// 機体ごとに測って決めること。
    pub check_eom_residual: f64,
    /// **立脚の重みを 0/1 から連続へ変える時間** [s]。`0` で無効（従来どおり）。
    ///
    /// 離地の手前で 1 → 0、着地の直後に 0 → 1 へこの時間かけて動かす。
    /// 重みは摩擦錐の力の上限（`w · f_max`）と「立脚足は動かない」の行に
    /// 掛かるので、**優先度 0 のランクが 1 周期で飛ばなくなる。**
    ///
    /// # なぜ要るのか
    ///
    /// 0/1 で切り替えると、遊脚の「力は厳密に 0」という硬い等式が 1 周期で
    /// 現れたり消えたりする（1 脚あたり 3 行）。階層 QP は優先度 0 を最小
    /// 二乗で解くので、そこで**どの層も満たさない解**を返すことがある。
    /// keel の実測では踏み替えのたび（trot の半周期 0.2 s ごと）に出て、
    /// 接地力の合計 0.1 N（体重 523 N）に対して胴体の角加速度 57 rad/s²、
    /// 前置トルクが 145 N·m/周期 跳んだ。
    ///
    /// **予定表は歩容の位相時計から採る。** MPC の窓でも同じことはできるが、
    /// あちらは 30 ms 刻みで制御周期 5 ms を解像できず、CHAMP には無い。
    ///
    ...
    /// 足は `w × この倍率 × 体重` までしか踏めない。既定 2.0。
    pub contact_force_cap_frac: f64,
...
pub struct GaitTuning {
    ...
    /// 立ち高さの大きいほう（keel 0.337）。指定すればそのまま使う（低くもできる。
    /// 床に近づきすぎれば計画時の検査で「組めません」）。**高くすると立脚・浮かせる脚とも伸びた
