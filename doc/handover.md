...
| **MPC 歩容** | `[gait] controller = "mpc" \| "centroidal"` で `quadruped_gait` の SRBD MPC / 重心 MPC を選べる（2026-09-06）。**質量・慣性・重心はモデルから入れる**（既定のままだと Cheetah 級の 9 kg で走る）。WBC の参照が準静的な自前のものから MPC の予測に変わる。**位置出力では効く**（既定歩容で crawl +0.436 → +0.476 m、trot +0.590 → +0.659 m、crawl のヨーのずれ 19.4° → 8.9°）。`gait.mpc_capture_point_gain_s` は 0 に落とすこと（articara も 0）。`mpc_observe_pose`（既定 true）で推定した高さ・roll・pitch を MPC の現在状態に入れる（quadruped-gait a7a531e。Cargo.lock はその rev）。`estimator = "kalman"` で legged_control の 18 状態 LKF に替えられる（横ずれが半分以下、既定は脚オドメトリ）|
...
| **WBC** | 既定は無効（`[wbc] enabled = false`）。有効にすると脚 12 軸が階層 QP の解に変わり、出力はトルク / 速度 / 位置から選ぶ。**実用は `output = "position"`**（位置 PD が歩容の目標を追い、WBC の τ をその上に前置として足す legged_control の hybrid joint。articara が namiashi で検証した構成と同じ）。歩容を詰めれば MuJoCo で trot 0.80 m/s を 95 %、walk 0.33 を 94 %、crawl 0.17 を 104 % で追従する。**トルク出力も legged_control の hybrid**（`τ_WBC + kp(q*−q) + kd(q̇*−q̇)` をホスト側で足す）で、遊脚は直交空間のタスク（`J_足·q̈ = kp(p*−p) + kd(ṗ*−ṗ) − J̇·v`、350/37）。**kd は 0.3（legged_control の 3 はモータ内 2.5 kHz の値。200 Hz のホスト側では発振して胴体が沈む。2026-09-06 に発見、それ以前のトルク出力の数字は無効）。** 直して 3 モデル（2.4 kg、実機質量 3.3 kg の 2 通り）× 5 走行で追従率 0.99〜1.06、転倒 0。**sim の時間軸のバグも同日に直した**（MuJoCo 2 ms × 3 = 6 ms を 5 ms と信じていた。それ以前の sim の数字は 20 % 歪み）。3.3 kg は連続定格では歩けず、MG4005 の 24 V 瞬時 2.5 N·m（`torque_scale = 1.6667`、`robots/namiashi.toml` に記入）で歩く。取り込んだ改善の採用可否と「指令通りに動くために分かったこと」は README。ゲインはすべて 2.4 kg のモデルで詰めたもの。位置・速度出力は 40 秒回しても崩れない。**実機のトルク制御には `torque_constant_nm_per_a` が要る**（無いと N·m が電流 A として線に乗る。`SerialPlant` が Torque を名乗らないので `run` は止まる）|
