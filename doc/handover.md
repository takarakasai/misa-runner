...
| **MPC 歩容** | `[gait] controller = "mpc" \| "centroidal"` で `quadruped_gait` の SRBD MPC / 重心 MPC を選べる（2026-09-06）。**質量・慣性・重心はモデルから入れる**（既定のままだと Cheetah 級の 9 kg で走る）。WBC の参照が準静的な自前のものから MPC の予測に変わる。**位置出力では効く**（既定歩容で crawl +0.436 → +0.476 m、trot +0.590 → +0.659 m、crawl のヨーのずれ 19.4° → 8.9°）。`gait.mpc_capture_point_gain_s` は 0 に落とすこと（articara も 0）。`mpc_observe_pose`（既定 true）で推定した高さ・roll・pitch を MPC の現在状態に入れる（quadruped-gait 側の改修。ローカル patch、未 push）。`estimator = "kalman"` で legged_control の 18 状態 LKF に替えられる（横ずれが半分以下、既定は脚オドメトリ）|
