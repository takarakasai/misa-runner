...
トルクにしてから**出す（`[wbc] joint_kp / joint_kd`、既定 5 / 3）。
両方 0 にすれば純粋なトルクになる。
...
|---|---|---|---|---|---|---|
| trot | 0.80 m/s | +9.302 m | +9.794 m | **+10.349 m**（103 %） | +2.321 m | **+4.070 m** |
| walk | 0.33 m/s | +3.830 m | +3.894 m | **+4.036 m**（98 %） | +0.648 m | **+1.048 m** |
| crawl | 0.17 m/s | +2.017 m | +2.231 m | +2.180 m（103 %） | +0.729 m | +0.150 m |

括弧は指令に対する追従率。**転倒はどれも無し。** 位置出力の追従率は
articara の `namiashi_tuned_gaits_hold` が押さえている 90〜110 % の帯に入る。

**トルク出力は legged_control の 2 点**（直交空間の遊脚タスクと hybrid
joint の弱い PD）**を入れて +0.28 → +2.3 m、MPC と併せて +4.1 m になった。**
それでも位置出力には届かない（trot で 41 %、walk 25 %）。
...
- **MPC 単体（WBC 無効）は trot を悪くする**（既定設定で +0.496 → +0.154 m）。
  ...
  いる）。
- **トルク出力は歩けるが位置には届かない**（trot 41 % / walk 25 %）。
  **MPC はここで効く** — trot +2.3 → +4.1 m でヨーのずれが 106° → 43°、
  walk +0.65 → +1.05 m でヨー 114° → 8°。crawl だけは逆に落ちる。
