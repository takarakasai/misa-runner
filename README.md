...
|---|---|---|---|---|
| 0.25 | 腿を振り出す（1 脚 6 段） | 0.8 s | 25 段 21.6 s | **1.9°** |
| 0.30（keel の既定） | 腿を振り出す | 0.8 s | 25 段 21.6 s | **1.6°** |
| 0.35 | ロールで倒して同時に折り返す（1 脚 5 段） | 0.5 s | 21 段 11.5 s | 5.7° |
| 0.40 | ロール | 0.5 s | 21 段 11.5 s | 3.1° |

- **0.337 m 以上なら**（床の余裕 3 cm 込み）hip のロールで脚を外へ倒し、腿と calf を
  同時に折り返す（`knee_flip_stand_overlap` で折り返しをロール中に 6 割進める）。速い。
- **それ未満は腿を胴体から離す向きへ振って**一直線を通す（`flip_legs`。2026-09-16 に
  実機で通った振り付け）。7 kg の腿をほぼ水平まで振る反動が胴体に出るので、
  **1 段は 0.8 s 未満にしない**（自動で上げる。0.30 m で 0.5 s だと 14°、0.8 s で 1.6°）。
