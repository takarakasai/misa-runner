...
中央へ。3 脚支持は静的に安定なので釣り合いの制御は要らず、1 段は
`knee_flip_stand_phase_s`（既定 0.5 s）と短くできる。keel を MuJoCo（kp 500 / kd 5）で
`<<` ↔ `>>`: **21 段 11.5 s、傾き 3° 以内**（0.4 s なら 9.2 s・6°）。畳んで振り出す
5 段だった頃は 25 段 24 s。車輪や腹が無い機体でも使える。**keel 実機で成功している
のはこの方式**（2026-09-16、5 段の版）。
...
0.8 s（0.65 で 9.8°、1.0 で 30°、0.5 で 25°）、脚の剛性が kp 1200 になると `<>` → `><`
の FR+RL で 29°。脚を浮かせる段の外乱は、浮かせる 2 脚が点対称な `<>` / `><` では
