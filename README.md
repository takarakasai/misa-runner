...
実機なので **デッドマン**（既定 1.0 s、`control.keys_deadman_ms`）が付く:
キーが来なければ速度だけ 0 に落とす
...
**デッドマンは端末のオートリピートの「最初の待ち」より長くすること。** リピートは
最初の 1 回だけ遅れて来る（X11 の既定は 660 ms 待ってから 25 Hz、Linux の
コンソールは 250 ms から 30 Hz）。デッドマンがその待ちより短いと、**押し続けて
いるのに一度速度が 0 に落ちて上がり直す = 脈動する**（2026-09-16 に実機で発生。
既定を 0.5 → 1.0 s にした）。人が連打して歩かせるときも、連打の間隔より長いこと。
落ちたときは WARN が 1 回ずつ出るので、脈動したらログを見る。
...
## 実測の一次遅れ（`gait.imu_gyro_lpf_hz` / `gait.estimator_velocity_lpf_hz`）
...
```toml
[gait]
imu_gyro_lpf_hz = 15.0             # 0 で無し（既定）。10〜20 Hz が目安
estimator_velocity_lpf_hz = 15.0   # 同上
```

一次遅れ `y += α(x − y)`、`α = dt/(τ + dt)`、`τ = 1/(2π f_c)`。最初の値は素通し。
掛かる先はジャイロが「脚オドメトリの回転項・世界座標の角速度（MPC）・WBC の
...
| | 無し | 15 Hz | 8 Hz |
|---|---|---|---|
| CHAMP + WBC 位置、kp 300 / kd 2 | +1.33 m | +1.32 m | +1.29 m |
| MPC + WBC 位置、kp 2000/2000/500 | +1.12 m | +1.14 m | +1.15 m |

実機では 15 Hz から。振動の切り分けは「帰還ゲインを 0 にして消えるか」と
「カットオフを下げて消えるか」の 2 軸で見る。

## 足音 = 早い着地（`sim` の「着地の衝撃」と `[hardware] mit_velocity_feedforward`）

`sim` の終わりに **着地の衝撃**が出る: 接地が false → true になった周期の足の
鉛直速度（平均 / 最大）と、着地後 50 ms の最大鉛直力。遊脚の山は sin² で終端速度 0
なので、**ここが 0 でなければ計画より早く地面に当たっている**。

keel（trot 0.12、kp 500 / kd 5）で調べた結果、着地速度はどの構成でも 0.5〜1.1 m/s、
ピーク力 140〜390 N（体重 523 N）で、これが実機の足音の正体。原因は 2 脚支持中に
胴体が対角線まわりへ落ちる（roll 2.5〜4.8°、pitch 1〜2.5° p-p）ぶん遊脚が 1〜2 cm
早く着くこと。**沈みではない**（重力補償で沈みを消しても速度は減らない）し、
**追従の遅れでもない**（下の速度前置でも減らない）。

| 変えたもの | 着地速度 前 / 後 [m/s] | ピーク力 前 / 後 [N] |
|---|---|---|
| 基準（重力補償 1.0） | 0.90 / 0.60 | 349 / 342 |
| **周期 0.40 → 0.35** | **0.57 / 0.55** | **180 / 329** |
| 周期 0.50 | 1.10 / 0.52 | 335 / 372 |
| 接地比 0.50 → 0.45 | 0.65 / 0.54（後脚 calf が変化率ゲートに当たる） | 244 / 389 |
| 遊脚 0.05 → 0.08 | 1.00 / 0.80 | 367 / 343 |
| kp 500 → 200（重力補償あり） | 1.67 / 0.73（roll 7.9°） | 230 / 342 |

揺れは周期の 2 乗で増えるので、**周期を詰めるのが唯一効いた**。根本策は遊脚終端の
形（ゆっくり下りて地面を探す）か、トルクからの接地推定で早着地を検出して足 z を
止めること（どちらも未実装）。

`[hardware] mit_velocity_feedforward = 0〜1`（既定 0）は歩容の目標角の 1 周期差分を
MIT の q̇_d に載せる（指令は Impedance になり、ブリッジは `joint_angular_velocity`
に、シムは位置 PD の q̇* に入れる）。追従の遅れは消えるが**足音には効かなかった**
（0.75 → 0.85 m/s）。残してあるのは追従の切り分け用。

## 膝の向きを実行中に反転する（`Intent.knee_pattern`、`[` / `]`、`--knee-script`）
...
膝を反転するには脚を伸ばし切る（特異点を通る）ので、その脚に荷重があっては
いけない。荷重を外す方法が 3 つ（`gait.knee_flip_style`）。どれも関節空間の
折れ線で、組む前に **全段が可動域の内側**で、**浮かせている脚の足先が浮かせ
始めた面より下がらない**ことを検査する（破れたら警告して何もしない）。

**`stand`（既定）: 立ったまま 1 脚ずつ。** 他の 3 脚で支え、胴体を反転する脚の
反対の対角へ `knee_flip_shift_m`（0.05）寄せて重心を支持三角形に入れ、その脚だけ
反転する。反転のしかたは `trot` と同じで、**hip のロールで脚を外へ倒しながら腿と
calf を同時に逆へ折り返す**（浮かす → 反転 → 戻す の 3 段。脚が一直線になる瞬間の
足先の下がりぶん、反転中だけ胴体を 0.34 m に上げる）。FL → RR → FR → RL、最後に
中央へ。3 脚支持は静的に安定なので釣り合いの制御は要らず倒れないが、**7 kg の脚を
振る反動と、荷重が 3 脚に移るときの立脚の沈みで胴体は揺れる**。1 段は
`knee_flip_stand_phase_s`（既定 0.65 s、4 脚で 15 s。5 段だった頃は 24 s）。

keel を MuJoCo で `<<` ↔ `>>`（反転中の胴体の傾きの最大。`sim` が最後に出す）:

| 1 段 | MIT 500 / 5 のまま | 反転中 hip / thigh を kp 2000 / kd 20 に |
|---|---|---|
| 0.5 s（11.5 s） | 16° | 8°（kd 40 なら 6°） |
| 0.65 s（15 s） | 14° | **3°** |
| 0.8 s（17 s） | 3° | 1.7° |

実機（2026-09-16、0.5 s、500 / 5）は体感 10° 揺れた。sim の 1 s ごとの表では 3° に
見えていたのは標本化の穴で、周期ごとの最大は 16°。**反転中だけゲインを上げる**のが
`[hardware.mit_gains_knee_flip]`（kp / kd を `[hip, thigh, calf]` で。`mit_gains` との間を
`mit_gains_ramp_s` 0.5 s で寄せる。calf は上限の 500 / 5 のまま）。車輪や腹が無い
機体でも使える。**keel 実機で成功しているのはこの方式。**

**シムの PD ゲインについて。** 指令に MIT の kp / kd が載る機体（`[hardware] kind = "ros2"`、
keel）は、MuJoCo の PD も**プロファイルの `mit_gains` で回る**。`sim --kp` / `--kv` は
ゲインが載らない機体（シリアル）だけに効く（`--kp 2000,2000,500` の関節別指定も同様）。
以前この README や手順書に書いた「kp 1200 / kv 30 での結果」は、実際には
プロファイルの 500 / 5 で回っていた。

**`trot`: 対角 2 脚ずつ（歩容の trot と同じ順）。** FL + RR → FR + RL。支えるのは
残りの対角 2 脚だけで、車輪には頼らない。点接触の足 2 つでは支持線まわりの
モーメントを接地力が作れないので**静的には釣り合わず**、しかも**角運動量は重力で
しか変わらない**（`I·θ̈ = m·g·(s + h·θ) − m·h·s̈`。胴体を速く横へ動かすと反作用で
逆へ回る）。keel は重心が支持線から 9 mm、支持線まわりの慣性 I_cm 1.7 kg·m²
（m·h² = 5.3 に対して小さい）、不安定極 4.7 rad/s で、重心を 4 mm 直すのに胴体が
5° 傾く。そこで 3 つを組み合わせる:

- **寄せる（4 脚のまま）**: 浮かせた姿勢の全身重心が支持線に乗るところまで胴体を
  寄せる（モデルから前置。脚の質量も動くので 3 回まわす）。
- **釣り合い**: 2 脚の間は毎周期、IMU の傾き θ・角速度と、**重心の支持線からの
  距離 s・その速度**の 4 状態を LQR で帰還して立脚の足先を支持線と直角にずらす
  （`knee_flip_balance_gains`、上限 `..._max_m` 0.10、変化率 `..._rate_m_s` 0.3）。
  s は立脚だけ実測の関節角（脚のたわみと胴体の位置）、浮かせた脚は目標角で
  出す（浮かせた脚の実測を使うと追従遅れが重心の速度に出て ṡ の項が指令を
  振り切る）。**変化率の上限が無いと上限の間でバンバンになり**、足先目標が
  数 ms ごとに跳んで足が滑り、片足を軸に yaw が回る。**上限 0.05 では張り付いて
  倒れる向きがある**（`<>` → `><` で 19°）。設計は `scripts/knee_flip_balance_lqr.py`。
- **2 脚でいる時間を短くする**: 浮かせながら hip のロールで脚を外へ倒し（可動域
  いっぱい、keel は 43°）、**腿と calf を同時に逆へ折り返す**（「浮かす → 反転 →
  戻す」の 3 段。腿を振り出してから伸ばす 5 段は要らない）。脚が一直線になる
  瞬間の足先の下がりは (上腿+下腿)·cos(ロール) で、これが立ち高さを超える機体
  では反転中だけ胴体を上げる（keel 0.426·cos 43° + 0.03 = 0.34 m）。着けたら、
  ずらしていた立脚から計画へ滑らかに繋ぐ（`PosePlayer::rebase`）。

keel を MuJoCo（実機と同じ kp 500 / kd 5、`robots/keel.toml`）で: **11 段 11.2 s、
`<<` → `>>` 7.7°、`>>` → `<<` 7.5°、`<>` → `><` 1.1°、`><` → `<>` 0.9°**（支持線まわりの
傾きの最大。開始時刻を変えても同じ）、yaw ±3°、接地は足だけ。`]` / `[` の巡回
`<<` → `<>` → `><` → `>>` では `<<` → `<>` と `><` → `>>` は片脚ずつ（3 脚支持、傾き 0°）、
`<>` → `><` が 4 脚とも反転する。**成立する範囲は狭い**: 1 段 `knee_flip_phase_s` は
0.8 s（0.65 で 9.8°、1.0 で 30°、0.5 で 25°）。脚を浮かせる段の外乱は、浮かせる 2 脚が点対称な `<>` / `><` では
打ち消し合い（帰還ゼロで 0.1°）、`<<` / `>>` では残る（帰還ゼロで 10°）。
`>>` へ寄せる量は FL+RR で 8 mm、`>>` になった後の FR+RL は 0.5 mm。

**実機の重心はモデルとずれている前提で。** 前置も釣り合いも**モデルの重心**を見るので、
実機の重心が 1 cm ずれていれば、支持線に乗せたつもりの重心が外れて浮かせた脚の側へ
倒れる（4 mm で 5° 傾く系。2026-09-16 の実機はこれで倒れた可能性が高い）。そこで
`run` / `sim` は**立って止まっているとき**、関節トルクから各足の鉛直力を出し
（`foot_forces_from_torque`）、力の重み付き平均で重心の xy を測って 5 s ごとに 1 行出す:

```
実測の重心（関節トルクから、5 s 平均）: x -0.0151 y -0.0001 m / モデル x -0.0136 y +0.0000 … → gait.com_offset_body_m = [-0.0015, -0.0001]（鉛直力の合計 526 N / 体重 523 N）
```

その値を `gait.com_offset_body_m` に書くと、前置・釣り合いの重心に足される（MuJoCo では
差 1〜2 mm、鉛直力の合計と体重が合うのが確認）。実機（2026-09-16）は差 (−10.7, −12.1) mm、鉛直力の合計 401 N（体重の 77 %。減速機の摩擦が静止荷重の一部を持つと電流からのトルクは小さく出る。4 脚一律なら重み付き平均の位置には効かない）。**trot 方式を実機で試す前に必ず測る。**

**重心をその場で測る「探り」（`knee_flip_probe_*`、試験中・既定 OFF）。** 設定値を
持たずに済ませるため、浮かす前に対角 2 脚を少しだけ浮かせて保持し、重力で倒れ始める
角加速度 α から `x = α·I_line/(m·g)` で重心の支持線からのずれを測り、寄せ直してから
本番に入る振り付け（上げ → 保持 → 下げ → 寄せ直し、`knee_flip_probe_rounds` 回）。
MuJoCo（kp 500）ではまだ測れていない: kp 500 の脚は荷重で 1 cm 余り沈むので 5 mm
では足が離れず、3 cm・1.0 s で上げても上げの反動と沈みで、モデルが正しいときにも
−10〜−15 mm と読み、注入した ±16〜28 mm を追わない。保持中に 17° まで倒れる回もある。
探りを入れると本番の反転も 28° まで悪化する（0 回なら 8° 以内のまま）。

**ただし立脚が震える。** 指令 `want` が毎周期 ±数 mm〜2 cm 跳ね（速度の項が
1 周期ごとの揺れを拾う。ジャイロは 1 周期ごとに ±7 mrad/s、ṡ は差分）、変化率制限が
それを鋸波にして立脚に出る（2.4 s に方向反転 70〜160 回）。指令に一次遅れを入れる
（`knee_flip_balance_lpf_hz` 2〜10 Hz）、ṡ の一次遅れを 10 Hz にする、速度の項の
ゲインを半分にする、のどれも震えは消えるが倒れる向きが出る = **位相余裕がほとんど
無く、震えが立たせている**。膝の反転中に推定器を毎周期 reset していてジャイロの
一次遅れ（`gait.imu_gyro_lpf_hz`）が効いていなかったのは直した（`sim` / `run`）が、
それでも同じ。**実機に持ち込める状態ではない。** 効きそうなのは hip の kd を上げて
脚の横方向の減衰を増やすこと（kp 500 / kd 20 なら `<>` ↔ `><` は一次遅れ 5 Hz 込みで
震えず 5° 以内。`<<` ↔ `>>` はまだ倒れる）と、浮かせた脚を反動の質量に使うこと。

**`rest`: 胴体を車輪・腹に載せて 4 脚とも浮かせ、まとめて。** `knee_flip_rest_height_m`
（載ったときの胴体高さ）まで 3 段で鉛直に下ろし、足を `knee_flip_foot_lift_m` 浮かせ、
反転する脚をまとめて反転、足を着けて 3 段で上げる。15 段。低い胴体では脚の曲げ限界で
足を hip の真下に置けない向きがある（keel の `>>` は腿の前限界 −1.57 rad と calf の
+2.18 rad）ので 2 つの手を使う:

- **hip のロールで脚を外へ倒してから腿を振る。** 脚の面が傾き、腿が真下を通る瞬間の
  膝の下がりが cos(ロール) 倍になる（keel はロール 44° で膝が床から 7 mm）。
- **車輪から立ち上がれる足の位置を、その向きの立ち位置にする。** 低い胴体で
  「可動域の内側で膝が床から 3 cm 浮く」足の位置をロールと前後・左右のずらしで
  探し（keel の `>>` は外へ 8 cm）、そこを `nominal_foot_body` に足す
  （`Controller::stance_xy_offset`。起動時と反転のたびに向きごとに決まる）。
  足を置き替える段が要らなくなる。

keel を MuJoCo で `<<` ↔ `>>`: 15 段 10.4 s、胴体 0.155 m で車輪に載り、**roll p-p 0.0° /
pitch p-p 1.0°**、転倒なし。接地するのは足と車輪だけ。`>>` の広い立ち位置（hip
ロール 26°）で trot 0.12 は +1.54 m（128 %）だが後脚の遊脚が 0.11 m と高く着地が
強い（500 N。未調査）。

伸ばすときに脚を向ける方向は `knee_flip_out_z_m`（hip から足先までの下がり、
既定 0 = 水平）を望みとし、腿の可動域と床の余裕を満たす角を望みに近い順に探す
（前へ振れなければ後ろへ）。反転中は浮かせている脚が立脚フラグから外れるので、
重力補償の前置は残りの脚に配られる。終わると `gait.knee_pattern` が替わり、歩容の
IK もその向きになる。`check` は 4 つの向きそれぞれで立ち姿勢に IK が届くかを出す。

向きごとの歩き（keel、trot 0.12、kp 500 / kd 5、重力補償 1.0）: `<<` +0.98 m、
`<>` +1.21 m、`><` +1.12 m、**`>>` +1.44 m（120 %、直進、後脚の着地力も半分）**。

## 前置トルクは階段状に入れない（`control.gravity_feedforward_blend_s`）

開ループの重力補償は**計画上の接地**に体重を配る。接地を 0/1 で切り替えると、
脚が離地・着地するたびに τ_ff が階段状に変わる。keel の trot で **1 周期に
41 N·m**（体重の半分ぶんが calf のモーメント腕に乗る）。歩容の出入りでも
0 ↔ 満載で同じ段差になり、実機では「歩容開始直後・終了直前にビクンとなる」
として出た（2026-09-14）。**目標角は滑らかなので、追従誤差にも
「目標角の最大変化率」にも出ない。**

そこで脚ごとの接地の重みを 0 → 1 へ鈍らせ（既定 0.1 s）、その比で体重を配る。
脚自身の重力ぶん（遊脚にも要る項）は脚ごとには割れないので、脱力 → 通電の
1 回だけ同じ時定数で入れる。`sim` の終わりに出る「前置トルクの最大段差」で測る。

keel、trot 0.12、kp 500 / kd 5、重力補償 1.0:

| 鈍らせ | τ_ff の段差 | 前進 / 10 s | 着地 前 / 後 | 胴体 roll p-p |
|---|---|---|---|---|
| 0（従来） | 41.1 N·m/周期 | +0.40 m | 0.92 m/s・350 N | 2.5° |
| 0.05 s | 4.2 | +0.53 m | 0.24・315 N | 2.0° |
| **0.10 s（既定）** | **2.3** | +0.48 m | **0.28・137 N** | **1.4°** |
| 0.20 s | 2.3 | +0.24 m（遅れて効かない） | 0.19・161 N | — |

**足音（早い着地）にも効く。** 接地力の配分が跳ばないぶん胴体の上下が減り、
遊脚が地面へ運ばれる速さが落ちる（trot の前脚で 0.92 → 0.28 m/s、350 → 137 N）。
walk も 247 → 167 N。crawl は元から段差が小さく、差は出ない。

**WBC の τ は別**（QP が毎周期解き直すので接地集合が変わると解も変わる）。
こちらは未対応。

## 止めるときに目標角を飛ばさない

歩容の位相生成は**速度指令がちょうど 0 になった瞬間**に「全脚接地・位相 0」へ
分岐する（`quadruped_gait::PhaseGenerator`）。遊脚の足が空中にいると、その目標が
1 周期で接地位置へ飛ぶ。`Controller::ramp_velocity` は速度を落としきったあと
**そのとき空中だった脚が着地するまで**微速（1 mm/s）で歩かせてから 0 に落とす。

**待つ条件は「いま空中の脚が着地したか」で、「4 脚とも接地」ではない。** trot
（接地比 0.5）は 4 脚が同時に接地する瞬間が無いので、全脚接地を待つ作りでは
`gait.stop_settle_s` の時間切れまで待って結局飛んでいた。空中の脚が着地すれば
残りの脚は離地したばかりで足はまだ地面の高さにいるので、どの足も飛ばない。

keel の実機で「`w` を一瞬押して止めると calf / thigh が急峻に動く」として出た
（2026-09-14）。MuJoCo（trot 0.12、`--vx-script "0:0.12,3:0"`）で目標角の最大
変化率が **61.3 → 4.9 rad/s**（歩いているときと同じ）。walk 5.1、crawl 6.2。

`sim` の終わりに出る「目標角の最大変化率」は**安全ゲートに入る前の生の指令**で、
追従誤差では見えない「指令が飛んだ」を測る。`--vx-script "0:0.12,3:0,7:0.12,9:0"`
で歩く・止まるを繰り返せる。

## 姿勢を置いて描く（`sim --hold-start`）

`--hold-start` を付けると `sim` は初期姿勢（`control.start_pose`）で止まったまま回る。
モデルに `[[pose]]` を足して `start_pose` に指すと、その姿勢を MuJoCo に置いて
**どこが床に着くか**（終わりの「足以外が接地しています」）と見た目（`--video`）を
確かめられる。keel の膝前向きの脚を目一杯曲げた姿勢で、腿が真下なら膝の筐体、
腿を 43° 前へ振ると足先、hip をロールすると車輪だけが着く、と分かった
（keel-runner `doc/knee_forward_fold_render.png`）。

## 開ループの重力補償（`[control] gravity_feedforward`）

WBC を使わずに、**計画上の目標角と計画上の接地だけ**から前置トルクを作って
指令に添える。MIT の機体（ブリッジ越し）では τ_ff、シムでは位置制御の前置。
`off`（既定）/ `legs`（脚リンクの重力だけ）/ `full`（脚の重力 + 体重を接地脚に
等分した接地力。`τ = τ_g(q) − Jᵀ f`、胴体は水平と仮定）。

```toml
[control]
gravity_feedforward = "full"
gravity_feedforward_scale = 1.0   # 実機で段階的に入れるなら 0.25 → 0.5 → 0.75 → 1.0
```

`gravity_feedforward_scale` は前置トルク全体に掛ける倍率（範囲 [0, 1.5]、既定 1.0）。
1 未満なら足りないぶんを MIT の PD が持つので、従来の挙動（0）と自重どおり（1）の
間を連続に渡れる。`check` と `run` の起動ログに倍率が出る。

**実測は一切見ない**（関節角も IMU も脚オドメトリも入らない）ので、ホスト側に
帰還ループは増えず、歩容は開ループのまま。WBC の位置出力との違いはそこ —
あちらは胴体の姿勢・高さ・速度の PD が τ_ff に乗り、運動方程式も実測の q・q̇ で
...
|---|---|---|---|---|
| 300 / 300 / 300 / 2 / 2 / 2 | −0.30 m | +0.58 m | −0.26 m | +1.33 m |
| 1200 / 1200 / 500 / 12 / 12 / 5 | +0.42 m | +1.07 m | | +1.21 m |
| 2000 / 2000 / 500 / 20 / 20 / 5 | +0.52 m | +1.07 m | | +1.16 m |

立ち高さ（指令 0.300）は kp 300 で 0.308 → 0.323 になり、PD だけで自重を持って
いた沈みが消える。`legs` は体重を持たないので効かない。yaw は帰還が無いので
流れる（kp 300 で +7.5° / 10 s）。`check` が「重力補償（開ループ）: … 体重 N」
を出す。`BodyEstimator::gravity_feedforward` は接地力の推定
`foot_forces_from_torque` と同じ式の逆なので、往復で体重の 1/n が戻ることを
試験している。
