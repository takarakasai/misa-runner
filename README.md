...
`knee_flip_shift_m`（0.05）寄せて重心を支持三角形に入れ、その脚だけ反転する。反転の
しかたは `trot` と同じで、**hip のロールで脚を外へ倒しながら腿と calf を同時に逆へ
折り返す**（浮かす → 反転 → 戻す の 3 段）。FL → RR → FR → RL、最後に中央へ。
3 脚支持は静的に安定なので釣り合いの制御は要らず倒れないが、**7 kg の脚を振る反動と
荷重が 3 脚に移るときの立脚の沈みで胴体は揺れる**。それを抑えるのが 3 つ:

- 反転中だけ hip / thigh を kp 2000 / kd 20 に（`[hardware.mit_gains_knee_flip]`）。
- **胴体を脚長の 94 %（keel 0.40 m）に上げ、脚が伸びた状態から反転する**（`knee_flip_stand_height_m`。
  ユーザー案）。calf の振りが 3.5 → 1.4 rad と短くなって反動が減り、一直線の瞬間の
  床余裕も 6 cm に増える。
- その余裕で、ロールを倒しながら折り返しを 6 割まで先に進める（`knee_flip_stand_overlap`）。

keel を MuJoCo（`keel_gravity.toml`、MIT 500 / 5）で `<<` ↔ `>>`（反転中の傾きの最大）:

| 1 段 `knee_flip_stand_phase_s` | 反転 | 胴体 0.40 + 重ね 0.6 + 反転中 kp 2000 | 胴体 0.34、kp 2000 | 0.34、MIT 500 / 5 のまま |
|---|---|---|---|---|
| 0.4 s | 9.2 s | **3.4°** | — | — |
| 0.5 s（既定） | 11.5 s | **3.1°** | 8° | 16° |
| 0.65 s | 15 s | 2.0° | 3° | 14° |
| 0.8 s | 17 s | — | 1.7° | 3° |

実機（2026-09-16、0.5 s、0.34、500 / 5）は体感 10° 揺れた（sim の 1 s ごとの表の 3° は
標本化の穴、周期ごとの最大は 16°）。5 段だった頃は 25 段 24 s。車輪や腹が無い機体でも
使える。**keel 実機で成功しているのはこの方式。**
