...
**何脚ずつ反転するか**は `;` で巡回でき（1 脚ずつ = stand → 対角 2 脚 = trot →
4 脚まとめて = rest。`Intent.knee_flip_style`）、**1 段の時間**は `=` / `-` で ±0.05 s
（0.2〜2.0 s。`Intent.knee_flip_phase_s`、いま選んでいるやり方の値を置き替える）。
どちらも反転の最中に押した分は次の反転から効く。rest はプロファイルに
`knee_flip_rest_height_m` が無ければ切り替わらない（WARN）。**反転はどのやり方でも
`r` / `f` で決めたいまの立ち高さのまま**行う（記録形式 8）
...
**`stand`: 立ったまま 1 脚ずつ。** 他の 3 脚で支え、胴体を反転する脚の反対の対角へ
`knee_flip_shift_m`（0.05）寄せて重心を支持三角形に入れ、その脚だけ反転する。
FL → RR → FR → RL、最後に中央へ。3 脚支持は静的に安定なので釣り合いの制御は要らない。
**反転は `r` / `f` で決めたいまの立ち高さのまま行う**（2026-09-17。以前は脚長の 94 %
= keel 0.40 m まで上げていたが、脚を伸ばし切った姿勢は実機で不安定だった）。

1 脚ぶんの並びは **寄せる → 抜く → （折り返し）→ 着ける**。「抜く」「着ける」は
**4 脚 ↔ 3 脚の荷重の移し替えだけに 1 段まるごと使う**（2026-09-17。実機で「3 脚支持に
移行する際に大きく揺れる」。それまでは膝を畳む動きが足も持ち上げていて、130 N が
0.20 s で移っていた。いまは 0.67 s）。前置トルクの配分も**計画の足の高さから作った
連続の重み**（`ControlOutput::contact_weight`、床から 2 cm 上で 0）で渡すので、足が
まだ床にいるうちに 3 脚ぶんへ配り替わって突き上げることがない（τ_ff の段差 0.5 N·m/周期）。
さらに **IMU で胴体の傾きを保つ**（`knee_flip_level_kp`、既定 1.0。反転を始めたときの
傾きを基準に、立脚の足先を `Δz = kp·(Δroll·y − Δpitch·x)` だけ上下させる。0 で切れる）。

高さで折り返しの経路が変わる（脚が一直線になる瞬間の足先の下がりは
(上腿+下腿)·cos(ロール)。keel は 0.311 m）:

| 立ち高さ | 経路 | 1 段 | 反転 | 反転中の傾きの最大 |
|---|---|---|---|---|
| 0.25 | 腿を振り出す（1 脚 7 段） | 0.8 s | 33 段 28.0 s | **1.7°** |
| 0.30（keel の既定） | 腿を振り出す | 0.8 s | 33 段 28.0 s | **1.6°** |
| 0.30 | 腿を振り出す | 0.65 s | 22.7 s | 3.0° |
| 0.30 | 腿を振り出す | 0.5 s | 17.5 s | 4.9°（荷重を抜く段が無かった頃は 14°） |
| 0.35 | ロールで倒して同時に折り返す（1 脚 6 段） | 0.8 s | 25 段 21.6 s | 1.5° |

- **0.337 m 以上なら**（床の余裕 3 cm 込み）hip のロールで脚を外へ倒し、腿と calf を
  同時に折り返す（`knee_flip_stand_overlap` で折り返しをロール中に 6 割進める）。
- **それ未満は腿を胴体から離す向きへ振って**一直線を通す（`flip_legs`。2026-09-16 に
  実機で通った振り付け）。7 kg の腿の反動が胴体に出るので 1 段は
  `knee_flip_swing_min_phase_s`（0.65 s）を下回らない。速くしたいなら `r` で胴体を
  上げてロールの経路に入れる。
- 反転中だけ hip / thigh を kp 2000 / kd 20 に（`[hardware.mit_gains_knee_flip]`）。
- `knee_flip_stand_height_m` を書けばその高さで固定できる（届かなければ 5 mm 刻みで下げる）。
- 反転中の様子は `MISA_FLIP_TRACE=<間引き>`（既定 10 = 20 ms ごと）で 1 行ずつ出る:
  段・立脚・接地の重み・計画の足の高さ・傾き・関節トルクからの鉛直力・水平保持の Δz。

5 段だった頃は 25 段 24 s。車輪や腹が無い機体でも使える。**keel 実機で成功しているのは
この方式。**
...
keel を MuJoCo（`keel_gravity.toml`、MIT 500 / 5）で `<<` ↔ `>>`: 1 段 0.8 s で 11 段 11.2 s、
支持線まわりの傾き 7.7° / 7.5°（`<>` ↔ `><` は 1° 前後）。`]` / `[` の巡回
`<<` → `<>` → `><` → `>>` では `<<` → `<>` と `><` → `>>` は片脚ずつ（3 脚支持、傾き 0°）、
`<>` → `><` が 4 脚とも反転する。**`<<` で `[` を押すと巡回が回って `>>`**（対角 2 脚ずつの
本物の 2 脚支持）になる。

**速くする 3 点（2026-09-17）。** ユーザー案の 3 つを同時に入れて 11.2 s → **7.0 s**:

1. **立って測った重心を前置に使う（自動）。** 立って止まっている間、関節トルクから
   出した 4 脚の鉛直力の重み付き平均（= CoP = 重心の真下）とモデルの重心の差を 1 s の
   一次遅れで持ち、`]` / `[` を押した瞬間に `knee_flip_com_est` の初期値にする。寄せる量と
   釣り合いの s0 がこれで直る。MuJoCo にモデルの重心ずれ 16 mm / 28 mm を注入しても
   推定が打ち消し、傾き 5.4°（無しでは 28°）。**胴体が動いている間の CoP は使えない**
   （`knee_flip_cop_tau_s`、既定 0: 残差 ±50 mm で推定が発散し、正しい重心でも 27°）。
2. **反転中だけ hip / thigh のゲインを上げる**（`[hardware.mit_gains_knee_flip]` 2000 / 20）。
   浮かせた脚の追従が速くなり、段を短くしても脚が遅れない。
3. **ロールで倒しながら折り返しを始める**（`knee_flip_reverse_overlap` 0.2）。浮かす段の
   終わりに腿と calf を着地姿勢へ 2 割進めておく。0.3 以上はロールが倒れ切る前に脚が
   伸びて足先が床に近づく（計画時の検査で「組めません」）。**胴体を上げて脚を伸ばした
   状態から始めれば重ねを増やせる**が、trot では釣り合いの効率（腰の寄せで重心が動く
   割合 I_cm / (I_cm + m h²)）が落ちて逆効果: `knee_flip_height_m` 0.345 で 8°、0.36 で
   15°、0.38 以上は転倒。trot の胴体は 0.337（最小）のまま。`stand` では効く（上）。

| 1 段 `knee_flip_phase_s` | 1 回の反転 | 支持線まわりの傾き | 反転中の胴体 roll / pitch |
|---|---|---|---|
| 0.8（旧） | 11.2 s | 7.7° / 7.5° | 5° / 6° |
| 0.6 | 8.4 s | 5.9° / 5.4° | 4.4° / 6.6° |
| **0.5（既定）** | **7.0 s** | **5.8° / 4.5°** | 4.3° / 3.9° |

0.5 で重心ずれ 16 / 28 mm を注入しても 5.4° / 3.5°、`]` の巡回は 7 + 7 + 7 s・4.3°。
**成立範囲が狭いのは変わらない**（釣り合いの指令は変化率上限でバンバンしている。
`>>` へ寄せる量は FL+RR で 8 mm、`>>` になった後の FR+RL は 0.5 mm）。

**寄せる段の z の跳び（2026-09-17 に修正）。** 実行中に立脚の足先をずらす上書きが、寄せる段の
足先 z にその段の床の高さ（−h_flip）を使っていたため、胴体を上げる途中の最初の周期で
目標が数 cm 跳んでいた（calf 90 rad/s。実機なら飛び上がる。比較動画で見えた）。
z は再生中の計画の値を使うようにした。以下の数字はその修正後。

**足を床に滑らせる折り返し `knee_flip_reverse_style = "slide"`（既定、ユーザー案 2026-09-17）。**
反転する 2 脚を浮かせず、**足先を床の上に置いたまま横（外）へ滑らせる**。ロールで倒れる
ぶん脚が伸びて足先が外へ動き（IK が届く所まで、keel で約 0.2 m）、いちばん外で一直線を
通して膝の向きを替え、同じ道を戻す。頂点だけ床の下 `knee_flip_slide_dip_m`（1.5 cm）を
目標にして一直線を確実に通す。反転する脚は `[hardware.mit_gains_knee_flip_swing]` で
柔らかく（kp 200 / kd 5）して床を押す力と摩擦を小さくする。足が床を支え続けるので
**2 脚支持の釣り合いの問題そのものが消える**（4 点で静的に安定）:

| | 1 段 0.8 s（9.5 s） | 1 段 0.5 s（5.9 s） |
|---|---|---|
| 支持線まわりの傾き最大 | **1.2°** | **1.8°** |
| 重心ずれ 16 / 28 mm を注入 | 1.2° / 1.2° | — / 1.5° |
| 釣り合いの帰還を切る | 0.7° | — |
| 反転する脚を 500 / 5 のまま | 0.4° | 1.0° |
| `]` の巡回 `<<` → `<>` → `><` → `>>` | 0.1° | 0.1° |

ロール方式（浮かせて釣り合う）の 4〜6°・重心ずれ 16 mm で 28° と比べて別物。胴体も
立ち高さ 0.30 のまま。実機での未知は**カーペットの摩擦**（sim は μ 0.7 で足が 0.2 m
滑る。柔らかい脚で床を押す力は数十 N）。以下の俯瞰はロール方式のもの。

**パラメータの俯瞰（2026-09-17、keel MuJoCo、`<<` ↔ `>>` の支持線まわりの傾きの最大）。**
既定（1 段 0.5 s、ロール方式、重ね 0.2、遊脚 0.02、釣り合い上限 0.10 / 0.3 m/s、
反転中 hip / thigh 2000 / 20、立って測った重心）が 3.6〜5.4°。1 つずつ変えると:

| 何を | 値 | 傾き | 読み |
|---|---|---|---|
| 1 段の時間 | 0.5 / 0.65 / 0.8 / 1.0 / 1.2 / 1.5 s | 4〜5 / 9〜21 / 6〜7 / 9〜11 / 8〜9 / 9° | **ゆっくりは効かない**。不安定極 4.7 rad/s に晒す時間が延びるぶん悪い。0.65 は毎回悪い（脚の振りの周期と胴体の揺れが合う） |
| 折り返しのやり方 | ロール（既定）/ 腿を振る `swing` | 5 / **19〜37°** | 腿をほぼ水平まで振る 7 kg × 2 の反動が支持線まわりに乗る。ロールは左右で打ち消し合う |
| ロールと折り返しの重ね | 0 / 0.2 / 0.3 | 7.5 / 5.4 / 5.2° | 0.3 以上は床の検査で通らない高さがある。0.2 |
| 遊脚の高さ | 0.01 / 0.02 / 0.04 / 0.06 | 4.5 / 5.4 / 6.2 / 7° | 低いほど脚の動きが小さい。0.02 は床余裕との折り合い |
| 胴体高さ（立脚の伸展） | 0.31 / 0.315 / 0.32 / 0.33 / **0.337** / 0.35 / 0.36 / 0.38 / 0.40 | 5.6 / 7.7 / 4.9 / 7.0 / 5.8 / 13 / 16 / 21 / 25° | 高いと釣り合いの効率 I_cm / (I_cm + m h²) が落ちる。**低い側は 0.31 で一直線の足先が床から 3 mm**（それ以下は組めない）で、0.31〜0.337 は同じ条件の繰り返しばらつき（±1.5°）の内。効率は 20 → 21.5 % しか変わらず、効くはずの 0.25（34 %）には届かない（`videos/knee_flip_trot_height_compare.mp4`） |
| 反転中の MIT ゲイン | 無し / 1000 / 2000 / 2000 (kd 40) | 31 / 21 / 7 / 8° | 浮かせた脚の追従。2000 / 20 |
| 釣り合いゲイン | ×0.7 / ×1.0 / ×1.3 | 16 / 7 / 5〜9° | 設計値のまま |
| 寄せの変化率 | 0.2 / 0.3 / 0.5 m/s | 6〜8 / 5〜7 / 14° | 0.3 |
| 寄せの上限 | 0.10 / 0.15 | 7.4 / 7.4° | 効かない（上限まで行かない） |
| 重心ずれの注入 | 16 / 28 / 42 mm | 5〜8° | 立って測った重心が打ち消す。押す前に立たなかった場合 26° |

**実機の注意。** 1 の重心の実測は、実機では減速機の摩擦で静止時 ±2 cm ぶれる
（2 回で x −24 / −44 mm、y −12 / 0 mm、鉛直力の合計 77 % / 108 %）。関節が動いて
いれば摩擦の向きが決まるので、`[` を押す前に `r` / `f`（立ち高さ ±）を 1 秒おきに
交互に押して、起動ログの「実測の重心」が 2〜3 回続けて ±3 mm で揃うのを見てから
押す。揃わなければ体重計で測って `com_offset_body_m` に書く。
...
**`pitch`: Pitch 軸（腿と calf）だけで一斉に折り返す。** hip のロールも足の滑りも使わない。
**全脚を伸ばし切る手前まで伸ばし**（足は床に着いたまま、胴体が脚長の手前まで上がる）、
そこで膝の向きを入れ替え、元の立ち高さへ戻す。反転しない脚も同じだけ伸び縮みするので
胴体は水平のまま。**伸ばす → 折り返す → 戻す の 3 段だけ**（1 段 0.5 s で 2.5 s）。

keel を MuJoCo（`keel_gravity.toml`、MIT 500 / 5 + 反転中 2000 / 20）で:

| | 値 |
|---|---|
| 胴体 | 0.300 → **0.421**（脚長 0.426 の 5 mm 手前）→ 0.300 m |
| `<<` ↔ `><` / `><` ↔ `>>` / `<<` ↔ `>>` | いずれも 3 段 2.5 s、**傾き 0.7° / 0.7° / 0.2°** |
| 接地中の足の滑り | 0.001〜0.002 m/s（足は動かさない） |
| 目標角の跳び | 無し（歩容開始の 31.8 rad/s だけ） |

`<<` → `><` → `<<` → `><` → `>>` → `><` の 5 回とも成功、転倒なし。立ち高さ 0.25 / 0.30 /
0.35 のどれでも（上がる先は同じ 0.421）、1 段 0.3〜0.8 s のどれでも 0.7°。**いまのところ
いちばん揺れないやり方**で、床の摩擦にも重心のずれにも依らない。

伸ばし切りは特異点（IK が解けず膝の向きも決まらない）なので
`knee_flip_straight_margin_m`（5 mm）手前で折り返す。そこでの膝の曲がりは腿 ±8.8° /
calf ∓17.6°、支えるのに要る膝トルクは 4 N·m ほど。**実機では胴体が 12 cm 上がる**ので
吊り具のたるみに注意。

**`all`: 反転する脚を一斉に（床を滑らせる）。** `trot` の滑らせる経路をそのままに、
**組に分けず、反転する脚を同時に**動かす。足を床から離さないので何脚を同時に反転
させても 4 点接地のままで、胴体を寄せる段も要らない（重心を動かさない）。`]` / `[` の
巡回はどの向きの組み合わせでも 2 脚か 4 脚が同時に反転する:

| 遷移 | 同時に反転する脚 | 段数 / 時間（1 段 0.5 s） | 反転中の傾きの最大 |
|---|---|---|---|
| `<<` ↔ `><` | 前の 2 脚 | 9 段 **2.5 s** | pitch 3.0° |
| `><` ↔ `>>` | 後ろの 2 脚 | 9 段 **2.5 s** | pitch 3.1° |
| `<<` ↔ `<>` | 後ろの 2 脚 | 9 段 2.5 s | pitch 2.7° |
| `<<` ↔ `>>` | **4 脚とも** | 9 段 2.5 s | pitch 2.0° |

keel を MuJoCo（`keel_gravity.toml`、MIT 500 / 5 + 反転中 2000 / 20）で
`<<` → `><` → `<<` → `><` → `>>` → `><` の 5 回とも成功、転倒なし、接地中の足の滑り
0.002 m/s、計画と接地のずれ 0 %。1 段 0.8 s なら 1 回 4.0 s・3.3°。**4 脚一斉がいちばん
揺れない**（4 本の反動が打ち消し合う）。`knee_flip_reverse_style = "slide"` が要る
（脚を浮かせると支える脚が無くなるので、`roll` / `swing` では計画時に断る）。
