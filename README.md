...
歩けるが、実機の質量 3.3 kg のモデルは 2.0 が要る**（下の表）。実機の
モータが瞬間的に出せる倍率を測って入れること。
...
| `namiashi_3p3_prop`（実機の質量、脚に比例配分） | 3.3 kg | 73 % | **2.0** | 0.98 / 0.98 m / 4.3° | **1.02** / 0.32 m / 1.2° |
| `namiashi_3p3_hip`（実機の質量、hip に集中） | 3.3 kg | 73 % | **2.0** | 1.02 / 0.06 m / 1.2° | **1.06** / 0.19 m / 0.2° |
...
上限に当たる）。`torque_scale = 2.0`（QP とアクチュエータの両方）で歩ける
ので、**実機のモータが連続定格の 2 倍を瞬間的に出せるかが trot 0.80 の
前提**。walk 0.33 は 3.3 kg でも定格内で歩く（+4.78 m、飽和 10 %）。
...
2. **脚オドメトリの誤差は 2 つ。** (a) 計画の立脚なのに浮いている足（着地・
   ...
   こちらの世界に近い。
3. **トルクの余裕が支配的。** 3.3 kg は連続定格では歩けず 2 倍で歩く。
   2.4 kg でも trot 0.80 で周期の 35〜40 % は上限に当たっている。実機の
   モータの瞬間トルク（`torque_scale`）を測ることが、制御の改良より先。
