...
| `torque` | τ だけ | トルク指令（`0xA1`）。**位置のループを完全に外す** |

**`position` が推奨なのは、それが legged_control の hybrid joint と同じ形
だから。** 関節の位置 PD が歩容の目標を追い、WBC の τ はその上に前置として
足される。articara が namiashi の WBC を検証したときの構成もこれで
（[`articara/tests/wbc_walk.rs`](../articara/tests/wbc_walk.rs)）、当初の
「PD を完全に外す」経路からわざわざ移してある。

理由は **WBC が出すのは加速度であって位置ではなく、位置の誤差を戻す積分器が
どこにも無いこと**。τ だけで駆動すると、モデル誤差のぶん関節がゆっくり
ずれ続ける（静止立位で高さが 0.6 mm/s 沈む）。位置 PD を残せばそれが
積分器の役をする。
...
出ないトルクを当てにする）。上げれば歩けるようになる話ではない — 上の表の
とおり進む量はむしろ落ちる。
...
`swing_height_m = 0.04`、`mpc_capture_point_gain_s = 0`）で:
...
括弧は指令に対する追従率。**転倒はどれも無し。** 同じ数字を articara の
`namiashi_tuned_gaits_hold` が 90〜110 % の帯で押さえている。
...
- **MPC 単体（WBC 無効）は trot を悪くする**（既定設定で +0.496 → +0.154 m）。
  ...
  いる）。
- **トルク出力は歩けない。** 詰めた設定でも 2〜13 % しか進まない（転びは
  しない）。理由は上の「3 つの出力の違い」— 位置のループを外すと積分器が
  無くなる。`wbc.torque_scale` で上限を 2〜3 倍にしても進む量はむしろ落ち、
  MPC の参照に替えても変わらない。
...
接地足は歩容の立脚フラグ（足裏センサが無いのでこれが唯一）。脚オドメトリ
（[`estimator.rs`](crates/misa-runner/src/estimator.rs)）は「接地していると
歩容が言っている足は世界に対して止まっている」という仮定だけを使う。世界座標
の絶対位置は出せないが、**支持足に対する相対位置**は出せて、転ぶかどうかに
効くのはそちらだけ。同じ推定を MPC 歩容も使う。
...
`MISA_WBC_DEBUG=1` で毎周期 1 行出る（接地・脚オドメトリ・姿勢・角速度・
要求した胴体加速度・解の胴体加速度・接地力の合計、それとトルクが定格の
9 割を超えた軸）。**要求と解の胴体加速度が一致していれば QP は仕事をして
