...
`--knee-style stand|rest`。

膝を反転するには脚を伸ばし切る（特異点を通る）ので、その脚に荷重があっては
いけない。荷重を外す方法が 2 つ（`gait.knee_flip_style`）。どちらも関節空間の
...
残りの対角 2 脚で、**静的には釣り合わない**（keel は重心が支持線から 9 mm しか
離れていない）。そこで胴体を `knee_flip_rest_height_m + knee_flip_trot_margin_m`
（既定は margin 0 = 車輪が着く高さ）まで下げ、**傾きを車輪で受け止める**。
keel を MuJoCo で `<<` ↔ `>>`: 24 段 17.6 s、胴体 0.16 m、roll p-p 4〜5° /
pitch p-p 9°、転倒なし。**margin を上げると大きく傾いてから車輪に落ちる**
（0.01 で 10°、0.02 で 27°）ので、0 のままにする。`stand` の半分の段数で済む。
